...
use minecraft_protocol::{components::{blocks as mc_blocks, chunk as mc_chunk}, ids::blocks::Block};
...
pub fn to_minecraft(sol_chunk: &sol_chunk::Chunk16) -> (mc_chunk::Chunk, Coordinate16) {
    ...
    let blocks = if byte_size >= 8 {
        ...
        let (palette, indexed, _bits_per_element) = sol_chunk.as_palette().unwrap();
        mc_chunk::PalettedData::Paletted { palette, indexed }
    };
